...
export const RbacMatrix: React.FC = () => {
  ...
  return (
    <section id="rbac" className="py-20 md:py-32 bg-[#08080A] border-t border-white/10 relative">
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            {ROLES.map(role => {
              ...
              return (
                ...
                >
                  ...
                  <div className="font-semibold text-base text-white">{role.name}</div>
                  <div className="text-xs text-arena-subtle mt-1">{role.badge}</div>
                </button>
              );
            })}

            <div className="p-4 bg-white/[0.02] border border-white/10 rounded-lg text-sm text-arena-muted">
          ...
          <div className="lg:col-span-8 bg-white/[0.02] border border-white/10 rounded-xl p-6 md:p-8">
            ...
            <div className="flex flex-wrap justify-between items-start gap-4 pb-4 border-b border-white/10 mb-6">
              <div>
                ...
                <div className="font-semibold text-2xl text-white mt-0.5">{activeRole.name}</div>
                <p className="text-arena-muted text-sm mt-1 max-w-xl">{activeRole.summary}</p>
              </div>
              <span className="px-3 py-1 bg-white/5 border border-white/15 text-white text-xs rounded-md font-medium">
                {activeRole.badge}
              </span>
            </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-5 h-5 rounded-full flex items-center justify-center text-xs ${
                      ...
                      {perm.allowed ? <Check className="w-3 h-3 stroke-[3]" /> : <X className="w-3 h-3 stroke-[3]" />}
                    </div>
                    <span className={`text-sm ${perm.allowed ? '' : 'line-through'}`}>
                      {perm.feature}
                    </span>
                  </div>

                  {perm.note && (
                    <span className="text-xs text-arena-lime bg-arena-lime/10 px-2 py-0.5 rounded hidden sm:inline">
                      {perm.note}
                    </span>
                  )}
                </div>
            ...
            <div className="mt-6 pt-4 border-t border-white/10 flex justify-between items-center text-xs text-arena-subtle">
